...
local get_gps = function()
  ...
  if not require("braint.functions").isempty(gps_location) then
    return require("braint.icons").ui.ChevronRight .. " " .. gps_location
  else
    return ""
  end
end
...
M.get_winbar = function()
  ...
  if not f.isempty(value) and f.get_buf_option "mod" then
    local mod = "%#LspCodeLens#" .. require("braint.icons").ui.Circle .. "%*"
    if gps_added then
      value = value .. " " .. mod
    else
      value = value .. mod
    ...
    vim.api.nvim_create_autocmd(
      { "CursorMoved", "CursorHold", "BufWinEnter", "BufFilePost", "InsertEnter", "BufWritePost", "TabClosed" },
      {
        ...
        callback = function()
          ...
          if not status_ok then
            require("braint.winbar").get_winbar()
          end
        end,
      }
    )
